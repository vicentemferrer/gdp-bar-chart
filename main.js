...
const xScale = d3.scaleUtc().domain([new Date(from_date), new Date(to_date)]).range([p, w - p])
const hScale = d3.scaleLinear().domain([d3.min(data, d => d[1]), d3.max(data, d => d[1])]).range([h, p])

const xAxis = d3.axisBottom(xScale)
const hAxis = d3.axisLeft(hScale)

svg.selectAll('rect').data(data).enter().append('rect').attr('x', d => xScale(new Date(d[0])))
  .attr('y', d => hScale(d[1]) - p).attr('width', 2).attr('height', d => h - hScale(d[1]))
